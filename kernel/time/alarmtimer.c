...
static void alarmtimer_enqueue(struct alarm_base *base, struct alarm *alarm)
{
	if (alarm->state & ALARMTIMER_STATE_ENQUEUED)
      ...
      // start remove print log for avoid the BT force crash  RAV-7992 zhiling.chen
      //pr_info("[oem alarm] %s: comm:%s pid:%d  exp:%llu func:%pf\n", __func__,
      //	current->comm, current->pid, ktime_to_ms(alarm->node.expires),
      //	alarm->function);
     // end remove print log for avoid the BT force crash  RAV-7992  zhiling.chen
...
static enum hrtimer_restart alarmtimer_fired(struct hrtimer *timer)
{
	...
	if (alarm_debug & 0x1) {
		pr_info("[oem alarm] %s: type:%d exp:%llu\n", __func__,
		alarm->type, ktime_to_ms(alarm->node.expires));
		alarm_debug &=0xFE;
	}
...
static int alarmtimer_suspend(struct device *dev)
{
	...
	if (min == 0)
		return 0;
	if(min_timer) {
		pr_info("[oem alarm]%s: [%p]type:%d  exp:%llu func:%pf\n", __func__,
		min_timer, min_timer->type, ktime_to_ms(min_timer->node.expires),
		min_timer->function);
	}
	alarm_debug = 0x1;
