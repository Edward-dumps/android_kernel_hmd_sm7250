...
#ifdef CONFIG_GROOT_CHARGE_FEATURE
#define DEMO_MODE_PARTITION  "/dev/block/bootdevice/by-name/oemowninfo"
#define DEMO_MODE_DATA_SIZE 32
#define DEMO_MODE_DATA_OFFSET 9376
#define DEMO_MODE_FLAG "enable"
#define DEMO_MODE_WORK_RECHECK_DELAY_1S 1000	/* 1 sec */
#define DEMO_MODE_WORK_RECHECK_DELAY_30S 30000	/* 30 sec */

bool is_demo_mode = false;
static int demo_mode_get_data_from_partition()
{
	int nResult = 0;
	struct file *filp = NULL;
	unsigned char demo_mode_data[DEMO_MODE_DATA_SIZE]={0};
	loff_t pos = DEMO_MODE_DATA_OFFSET;

	filp = filp_open(DEMO_MODE_PARTITION, O_RDONLY, 0640);
	if (IS_ERR(filp)) {
		pr_err("%s: Failed open file=%s, ret=%p\n", __func__, DEMO_MODE_PARTITION, filp);
		return PTR_ERR(filp);
	}
	nResult = kernel_read(filp, demo_mode_data, DEMO_MODE_DATA_SIZE, &pos);
	if(nResult){
		pr_info("%s: Demo mode read data success!\n", __func__);
	}
	else {
		pr_err("%s: Demo mode cannot read data!\n", __func__);
	}
	filp_close(filp, NULL);

	if (strncmp(demo_mode_data, DEMO_MODE_FLAG, sizeof(DEMO_MODE_FLAG)) == 0){
		is_demo_mode = true;
	}

	pr_info("%s: is_demo_mode = %d\n", __func__, is_demo_mode);

	return 0;
}

int demo_mode_write_partition_data(struct smb_charger *chg,
                const union power_supply_propval *val)
{
	...
	unsigned char demo_mode_data[DEMO_MODE_DATA_SIZE]={0};
	loff_t pos = DEMO_MODE_DATA_OFFSET;
	union power_supply_propval charging_val = {0,};

	pr_info("%s: is_demo_mode = %d, enable is %d\n", __func__, is_demo_mode, val->intval);
	filp = filp_open(DEMO_MODE_PARTITION, O_RDWR, 0664);
	if (IS_ERR(filp)) {
		pr_err("%s: Failed open file=%s, ret=%p\n", __func__, DEMO_MODE_PARTITION, filp);
		return PTR_ERR(filp);
	}
	...
	if ((1 == val->intval) && (false == is_demo_mode)) {
		nResult = kernel_write(filp, DEMO_MODE_FLAG, sizeof(DEMO_MODE_FLAG), &pos);
		if(nResult){
			is_demo_mode = true;
			schedule_delayed_work(&chg->demo_mode_monitor_work,
					msecs_to_jiffies(DEMO_MODE_WORK_RECHECK_DELAY_1S));
			pr_info("%s: Demo mode write data success, is_demo_mode = %d!\n", __func__, is_demo_mode);
		}
		else {
			is_demo_mode = false;
			cancel_delayed_work_sync(&chg->demo_mode_monitor_work);
			pr_err("%s: Demo mode write data error, is_demo_mode = %d!\n", __func__, is_demo_mode);
		}
	} else if (0 == val->intval) {
		is_demo_mode = false;
		nResult = kernel_write(filp, demo_mode_data, DEMO_MODE_DATA_SIZE, &pos);
		if(nResult){
			//enable charging
			charging_val.intval = 1;
			nResult = smblib_set_prop_battery_charging_enabled(chg, &charging_val);
			pr_info("%s: Demo mode clean data success, is_demo_mode = %d!\n", __func__, is_demo_mode);
		}
		else {
			pr_err("%s: Demo mode clean data error, is_demo_mode = %d!\n", __func__, is_demo_mode);
		}
		//cancel_delayed_work_sync(&chg->demo_mode_monitor_work);
	}
...
static void smblib_demo_mode_monitor_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
						demo_mode_monitor_work.work);
	int rc = 0;
	...
	if (first_check) {
		demo_mode_get_data_from_partition();
		first_check = false;
	}

	pr_info("%s: is_demo_mode = %d\n", __func__, is_demo_mode);
	if (is_demo_mode) {
		rc = smblib_get_prop_batt_capacity(chg, &prop_val);
		pr_info("%s: batt_capacity = %d\n", __func__, prop_val.intval);
		if (prop_val.intval >= 35) {
			//disable charging
			charging_val.intval = 0;
			rc = smblib_set_prop_battery_charging_enabled(chg, &charging_val);
		} else if (prop_val.intval <= 30) {
			//enable charging
			charging_val.intval = 1;
			rc = smblib_set_prop_battery_charging_enabled(chg, &charging_val);
		}

		/* recurse every 30 seconds */
		schedule_delayed_work(&chg->demo_mode_monitor_work,
			msecs_to_jiffies(DEMO_MODE_WORK_RECHECK_DELAY_30S));
	} else {
		cancel_delayed_work_sync(&chg->demo_mode_monitor_work);
	}
...
#endif /*CONFIG_GROOT_CHARGE_FEATURE*/
...
irqreturn_t usbin_uv_irq_handler(int irq, void *data)
{
	...
	smblib_dbg(chg, PR_MISC, "IRQ: %s VBUS: %d mV\n", irq_data->name, pval.intval/1000);
...
static void update_sw_icl_max(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB_CDP:
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true,
					CDP_CURRENT_UA);
		break;
	case POWER_SUPPLY_TYPE_USB_OCP:
...
static void typec_src_insertion(struct smb_charger *chg)
{
	...
	smblib_dbg(chg, PR_MISC, "%s start\n", __func__);
	if (chg->pr_swap_in_progress) {
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, false, 0);
		return;
	}
...
#ifdef CONFIG_GROOT_CHARGE_FEATURE
	INIT_DELAYED_WORK(&chg->demo_mode_monitor_work, smblib_demo_mode_monitor_work);
	schedule_delayed_work(&chg->demo_mode_monitor_work,
			msecs_to_jiffies(DEMO_MODE_WORK_RECHECK_DELAY_30S*2));
#endif /*CONFIG_GROOT_CHARGE_FEATURE*/
