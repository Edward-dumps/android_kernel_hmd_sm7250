...
int ili_switch_tp_mode(u8 mode)
{
	...
	case P5_X_FW_GESTURE_MODE:
		...
		if (ges_dbg) {
			...
			ili_set_tp_data_len(DATA_FORMAT_GESTURE_DEBUG, true);
		}
		break;
...
int ili_sleep_handler(int mode)
{
	...
	switch (mode) {
	case TP_SUSPEND:
		ILI_ERR("TP suspend start\n");
		if(ilits->tp_suspend == true) {
			ILI_ERR("Already suspend, do nothing\n");
			goto out;
		}
		ili_irq_disable();
		if (sense_stop) {
		...
		if (ilits->gesture) {
			...
			ILI_INFO("enable irq wake \n");
			enable_irq_wake(ilits->irq_num);
			ili_irq_enable();
		} else {
			if (ili_ic_func_ctrl("sleep", SLEEP_IN) < 0)
				ILI_ERR("Write sleep in cmd failed\n");
			if (ili_reset_ctrl(TP_HW_POWER_OFF) < 0)
				ILI_ERR("TP pull Reset down failed during suspend\n");
		}
		ILI_ERR("TP suspend end\n");
	...
	case TP_RESUME:
#if !RESUME_BY_DDI
		ILI_ERR("TP resume start\n");

		if (ilits->gesture) {
			ILI_INFO("disable irq wake \n");
			disable_irq_wake(ilits->irq_num);
		}
...
void tp_reset_for_lcd_use(void)
{
	ILI_ERR("TP reset by LCD \n");
	ili_tp_reset();
}
EXPORT_SYMBOL(tp_reset_for_lcd_use);

int tp_supend_need_power_reset_high(void)
{
	if (ilits == NULL) {
		ILI_ERR("No ilitek TP found \n");
		return 0;
	}
	else {
		ILI_ERR("gesture:%d \n", ilits->gesture);
		return ilits->gesture;
	}
}
EXPORT_SYMBOL(tp_supend_need_power_reset_high);

void tp_gesture_force_recover(bool force)
{
	if (ilits == NULL) {
		ILI_ERR("No ilitek TP found \n");
		return;
	...
	if (force) {
		if (ilits->gesture != ilits->gesture_record)
			ilits->gesture = ilits->gesture_record;
	} else {
		ilits->gesture = 0;
	}
	return;
}
EXPORT_SYMBOL(tp_gesture_force_recover);
...
int ili_fw_upgrade_handler(void *data)
{
	...
	if (ret != 0) {
		...
		if((ilits->headset_plug_status)&&(ilits->actual_tp_mode !=P5_X_FW_TEST_MODE)) {
			ret = ili_ic_func_ctrl("ear_phone", ilits->headset_plug_status);// plug in
			if(ret<0) {
...
int ili_set_tp_data_len(int format, bool send)
{
	u8 cmd[2] = {0}, ctrl = 0;
	u16 self_key = 2;
...
void ili_report_handler(void)
{
	...
	if (!ilits->report || atomic_read(&ilits->tp_reset) ||
		...
		ILI_DBG("ignore report request\n");
		return;
	}

	if (ilits->irq_after_recovery) {
		ILI_DBG("ignore int triggered by recovery\n");
	...
	if (ilits->actual_tp_mode == P5_X_FW_GESTURE_MODE) {
		...
		if (ilits->pm_suspend) {
			...
			if (!ret) {
				ILI_ERR("system(spi) can't finished resuming procedure, skip it");
				return;
			}
		}
	}
	...
	if (ret < 0) {
		...
		if (ret == DO_SPI_RECOVER) {
			...
			ilits->need_recovery = false;
/*
			if ( (0 == lcd_recovery_flag) && (ilits->tp_suspend == false) ) {
				ili_ic_get_ddi_reg_onepage(0x00, 0x0a, &ddi_data);
				ILI_INFO("ddi_data = %x\n", ddi_data);
				if(ddi_data != 0x9c) {
					lcd_recovery_flag = 1;
...
int ili_reset_ctrl(int mode)
{
	...
	case TP_HW_RST_ONLY:
		...
		ili_tp_reset();
		break;
	case TP_HW_POWER_OFF:
		ILI_DBG("TP HW POWER OFF\n");
		ili_tp_power_off();
		break;
	default:
